...
# CenSat annotations: two small BED files per sample (open access, HPRC S3 bucket)
while read -r key; do
  f="$CENSAT/$(basename "$key")"
  [ -s "$f" ] || curl -sSL --retry 5 -o "$f" "https://s3-us-west-2.amazonaws.com/human-pangenomics/$key"
